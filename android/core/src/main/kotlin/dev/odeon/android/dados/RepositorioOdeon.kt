...
class RepositorioOdeon(private val cofre: Cofre) {
    ...
    /// Os canais e o que está no ar. Lista vazia quando não dá — e a tela sabe
    /// dizer «nenhum canal» sem confundir com «não carregou» (§18).
    suspend fun canaisAoVivo(): List<CanalNoAr> = withContext(Dispatchers.IO) {
        runCatching { exigirApi().canaisAoVivo() }
            .onFailure { android.util.Log.w("Odeon", "os canais não vieram: $it") }
            .getOrDefault(emptyList())
    }

    /// A grade dos canais de fonte externa.
    suspend fun guiaAoVivo(horas: Int = 4): Guia? = withContext(Dispatchers.IO) {
        runCatching { exigirApi().guiaAoVivo(horas) }
            .onFailure { android.util.Log.w("Odeon", "o guia não veio: $it") }
            .getOrNull()
    }

    /// A grade que o próprio Odeon programa.
    suspend fun gradeDoOdeon(horas: Int = 5): GradeDoOdeon? = withContext(Dispatchers.IO) {
        runCatching { exigirApi().gradeDoOdeon(horas) }
            .onFailure { android.util.Log.w("Odeon", "a grade do Odeon não veio: $it") }
            .getOrNull()
    }

    /// Abre a sessão num canal.
    ///
